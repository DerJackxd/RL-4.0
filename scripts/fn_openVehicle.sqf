...
if(EQUAL(playerSide,civilian)) exitWith {hintSilent "Sie sind nicht berechtig das zu benutzen!"};
if(EQUAL(playerSide,independent)) exitWith {hintSilent "Sie sind nicht berechtig das zu benutzen!"};
if(isNull _curTarget) exitWith {hintSilent "Sie haben kein Fahrzeug ausgewaehlt"};
