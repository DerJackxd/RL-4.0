...
private["_display","_curTarget","_bUnrestrain","_bArrest","_bTicket","_bEscort","_bPutInCar","_bSearch","_bShowLicenses","_bDrunkenCheck","_bDrugCheck"];
if(!dialog) then {
...
_bDrunkenCheck = _display displayCtrl 2408;
_bDrugCheck = _display displayCtrl 2409;
...
//Set Drunk/Drug Test
_bDrunkenCheck ctrlSetText localize "STR_pInAct_drunkenCheck";
_bDrunkenCheck buttonSetAction "[life_pInact_curTarget] call life_fnc_drunkenCheck;";

_bDrugCheck ctrlSetText localize "STR_pInAct_drugCheck";
_bDrugCheck buttonSetAction "[life_pInact_curTarget] call life_fnc_drugCheck;";
...
if(!((player distance (getMarkerPos "police_hq_1") < 30) OR
  ...
  (player distance (getMarkerPos "jail_marker") < 80) OR
  (player distance (getMarkerPos "cop_spawn_9") < 80) OR
