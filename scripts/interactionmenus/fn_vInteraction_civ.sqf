...
if((typeOf _curTarget in ["C_Offroad_01_F"])) then
{
	...
	if(player getVariable "oelFassTransport") then {_Btn3 ctrlEnable true;}else{_Btn3 ctrlEnable false;};
	
	_Btn4 ctrlSetText "Fass abladen";
	_Btn4 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_oelFassVehicleAb;closeDialog 0;";
	if((!(player getVariable "oelFassTransport")) && ((_curTarget getVariable "oelSlot") >= 1)) then {_Btn4 ctrlEnable true;}else{_Btn4 ctrlEnable false;};
}else{
