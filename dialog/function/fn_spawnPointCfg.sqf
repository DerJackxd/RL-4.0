...
switch (_side) do
{
	case west:
	{
		_return = [
			...
			["cop_spawn_3","Brisbane","\a3\ui_f\data\map\MapControl\watertower_ca.paa"],
	...
	case civilian:
	{
		...
		if(life_isPlayer_alive) then
		{
			...
			if(LEVEL >= 1) then {
				_return = [
					["civ_spawn_1","Melbourne","\a3\ui_f\data\map\MapControl\watertower_ca.paa"]
				];
			};
			if(LEVEL >= 2) then {
	...
	case independent: {
				...
				_return = [
					...
					["medic_spawn_7","Darwin","\a3\ui_f\data\map\MapControl\hospital_ca.paa"]
				];
		};
	...
	case east: {
				...
				_return = [
					...
					["adac_spawn_2","ADAC Perth","icons\wrench.paa"],
					["adac_spawn_3","ADAC Brisbane","icons\wrench.paa"],
					["adac_spawn_4","ADAC Alice-Springs","icons\wrench.paa"]
				];
		};
};
