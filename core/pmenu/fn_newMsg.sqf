...
switch(_type) do
{
	...
	case 9:
	{
		if(({side _x == east} count playableUnits) == 0) exitWith {hint format["Zurzeit ist kein ADAC-Mitarbeiter im Dienst. Bitte probiere es sp�ter nochmal."];};
	...
	case 11:
	{
		...
		if(isNull life_aufTarget) exitWith {hint format["Keine Auftrag ausgw�hlt!"];};
		if(_msg == "") exitWith {hint "Sie muessen eine Nachrich eingeben!";};
