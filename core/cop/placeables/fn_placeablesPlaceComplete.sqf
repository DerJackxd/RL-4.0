...
if((typeOf life_barrier_activeObj) == "Land_Razorwire_F")then
{
	life_barrier_activeObj setDamage 1;
	[[life_barrier_activeObj],"TON_fnc_spikeStrip",false,false] call life_fnc_MP; //Send it to the server for monitoring.
}else{
	life_barrier_activeObj enableSimulationGlobal true;
};
life_bar_placey pushBack life_barrier_activeObj;
publicVariable "life_bar_placey";
