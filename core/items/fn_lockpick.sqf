...
if(playerSide == east OR playerSide == independent) then {
	...
	if(!_isVehicle) then {
		...
		_dice = random(100);
		if(skill_civ_twocker && (_curTarget isKindOf "LandVehicle")) then
		{
			if(_dice < 50) then {
				titleText[localize "STR_ISTR_Lock_Success","PLAIN"];
				life_vehicles pushBack _curTarget;
			...
			if(skill_civ_twocker && skill_civ_twocker1 && (_curTarget isKindOf "Air")) then
			{
				if(_dice < 50) then {
					titleText[localize "STR_ISTR_Lock_Success","PLAIN"];
					life_vehicles pushBack _curTarget;
