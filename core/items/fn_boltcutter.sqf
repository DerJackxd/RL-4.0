...
*/
private["_building","_door","_doors","_cpRate","_title","_progressBar","_titleText","_cp","_ui","_exp"];
_building = [_this,0,ObjNull,[ObjNull]] call BIS_fnc_param;
if(isNull _building) exitWith {};
if(!(_building isKindOf "House_F")) exitWith {hint "You are not looking at a house door.";};
if(isNil "life_boltcutter_uses") then {life_boltcutter_uses = 0;};
if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building) then {
	if(skill_civ_heister) then
	{
		_dice = random(100);
		if(_dice < 20) then
		{
			[[[1,2],"STR_ISTR_Bolt_heister",true,[]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		}else{
			[[[1,2],"STR_ISTR_Bolt_AlertFed",true,[]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
			[_building,"bankalarm"] call life_fnc_globalSound;
		};
	}else{
		[[[1,2],"STR_ISTR_Bolt_AlertFed",true,[]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		[_building,"bankalarm"] call life_fnc_globalSound;
...
if((nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
	...
	if(skill_civ_heister) then
	{
		...
		if(_dice < 20) then
		{
			...
			[[[1,2],"STR_ISTR_Bolt_AlertFed2",true,[]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
			[_building,"bankalarm"] call life_fnc_globalSound;
		};
	}else{
		[[[1,2],"STR_ISTR_Bolt_AlertFed2",true,[]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
...
if(skill_civ_heister) then
{
	switch (typeOf _building) do {
		case "Land_Dome_Big_F": {_cpRate = 0.006;};
		case "Land_Research_house_V1_F": {_cpRate = 0.0045;};
