...
if(_rip) then
{
    ...
    while{true} do
    {
        ...
        if(_cP >= 1) exitWith { [[1,player],"life_fnc_createMarker",west,false] spawn life_fnc_MP;};
        if(_robber distance _shop > 5) exitWith { [[1,player],"life_fnc_createMarker",west,false] spawn life_fnc_MP;};
		if(life_istazed) exitWith { [[1,player],"life_fnc_createMarker",west,false] spawn life_fnc_MP;};
		if(life_interrupted) exitWith { [[1,player],"life_fnc_createMarker",west,false] spawn life_fnc_MP;};
        if!(alive _robber) exitWith { [[1,player],"life_fnc_createMarker",west,false] spawn life_fnc_MP;};
		if((_robber getVariable["restrained",false])) exitWith { [[1,player],"life_fnc_createMarker",west,false] spawn life_fnc_MP;};
        
    }; // the loop continues til the progressbar is full, distance is exceeded or robber dies. 
    if!(alive _robber) exitWith { _rip = false; [[getPlayerUID _robber,name _robber,"46A"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP; [[1,player],"life_fnc_createMarker",west,false] spawn life_fnc_MP;};
	if(life_istazed) exitWith { _rip = false; [[getPlayerUID _robber,name _robber,"46A"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP; [[1,player],"life_fnc_createMarker",west,false] spawn life_fnc_MP;};
