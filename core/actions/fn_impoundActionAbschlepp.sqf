...
{	
	...
	{
		[[0,"STR_NOTF_Konif",true,[SEL(SEL(_vehicleData,0),1),_vehicleName]],"life_fnc_broadcast",true,false] call life_fnc_MP;
	}else{
		[[0,"STR_NOTF_KonfiA",true,[SEL(SEL(_vehicleData,0),1),_vehicleName]],"life_fnc_broadcast",true,false] call life_fnc_MP;
	};
	life_action_inUse = true;

	_upp = localize "STR_NOTF_Konfis";
	...
	if(player distance _vehicle > 10) exitWith {hint localize "STR_NOTF_KonfisCancelled"; life_action_inUse = false;};
	...
	if(EQUAL(count crew _vehicle,0)) then {
		...
		[[_vehicle,true,player,true],"TON_fnc_vehicleStore",false,false] call life_fnc_MP;
		waitUntil {!life_impound_inuse};
		hint format[localize "STR_NOTF_Konfid",_type,_price];
		[[0,"STR_NOTF_HasKonfid",true,[profileName,SEL(SEL(_vehicleData,0),1),_vehicleName]],"life_fnc_broadcast",true,false] call life_fnc_MP;
		ADD(BANK,_price);
	} else {
		hint localize "STR_NOTF_KonfisCancelled";
	};
};
