...
if(life_inv_messer > 0) then {
	...
	if(EQUAL(_displayName,"")) exitWith {life_action_inUse = false;life_action_gathering = false;};
	...
	while{true} do {
		...
		if(_cP >= 1) exitWith {life_action_gathering = false;};
		if(!alive player) exitWith {life_action_gathering = false;};
		if(isNull _animalCorpse) exitWith {life_action_gathering = false;};
		if(player != vehicle player) exitWith {life_action_gathering = false;};
		if(life_interrupted) exitWith {life_action_gathering = false;};
	};

	life_action_inUse = false;
	5 cutText ["","PLAIN"];
	player playActionNow "stop";
	if(isNull _animalCorpse) exitWith {life_action_gathering = false;life_action_gathering = false;life_action_inUse = false;};
	if(life_interrupted) exitWith {life_action_gathering = false;life_interrupted = false; titleText[localize "STR_NOTF_ActionCancel","PLAIN"]; life_action_inUse = false;};
	if(player != vehicle player) exitWith {life_action_gathering = false;titleText[localize "STR_NOTF_RepairingInVehicle","PLAIN"];};
