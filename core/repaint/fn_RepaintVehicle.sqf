...
closeDialog 0;
	...
	if (playerSide == east) then
	{
		...
		if(CASH < _basePrice) exitWith {hint "Sie haben nicht genung Geld um mich bezahlen zu koennen!"};

		if(isNil {_vehicleData}) exitWith {hint "Cheated car?."};
		if ((getPlayerUID player) != _vehOwner) exitWith {hint "Sie sind nicht der Besitzer!"};
